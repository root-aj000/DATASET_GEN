# core/skeleton.py
"""
Builds a PERFECTLY GROUPED skeleton of any size (multiple of BASE_UNIT).

GROUPING ORDER (top to bottom in the CSV):
  1. Theme      (Food → Fashion → Tech → ...)
...
def _build_object_block_template() -> List[Tuple[str, str]]:
    ...
    assert len(pairs) == ROWS_PER_OBJECT_PER_CYCLE
    return pairs
...
def _build_all_core_rows() -> List[Dict]:
    ...
    assert len(rows) == TOTAL_ROWS, f"Built {len(rows)} rows, expected {TOTAL_ROWS}"
    return rows
...
def _build_pool(items: Dict[str, int]) -> List[str]:
    ...
    assert len(pool) == TOTAL_ROWS, f"Pool size = {len(pool)}, expected {TOTAL_ROWS}"
    random.shuffle(pool)
    return pool
...
def verify_skeleton(df: pd.DataFrame) -> List[str]:
    ...
    expected_sent_per_obj = 5 * NUM_CYCLES
    expected_emo_per_obj = 3 * NUM_CYCLES
    ...
    # ── Theme × Sentiment ──
    expected_theme_sent = 30 * NUM_CYCLES
    ts_bad = 0
    for theme in THEME_ORDER:
    ...
    # ── Theme × Emotion ──
    expected_theme_emo = 18 * NUM_CYCLES
    te_bad = 0
    for theme in THEME_ORDER:
        t_df = df[df["theme"] == theme]
        for e in EMOTION_ORDER:
