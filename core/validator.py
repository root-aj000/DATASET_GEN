...
def _chk(df: pd.DataFrame, col: str, expected: dict, errors: List[str]):
...
def validate_columns_exist(df: pd.DataFrame, columns: List[str]) -> Tuple[bool, List[str]]:
    """
    Simple validation that specific columns exist.
...
def validate_column_filled(
    df: pd.DataFrame,
    column: str,
    min_length: int = 1,
) -> Tuple[int, int]:
    """
    Check how many rows have a column filled.
    
    Args:
        df: DataFrame
        column: Column name to check
        min_length: Minimum string length to consider "filled"
        
    Returns:
        Tuple of (filled_count, total_count)
    """
    if column not in df.columns:
        return 0, len(df)
    
    filled = sum(
        1 for val in df[column].fillna("").astype(str)
        if len(val.strip()) >= min_length
    )
